...
Perhaps most people would think of \code{GIF} images as the first choice for making animations, because it is well known that \code{GIF} is one of the only few image formats that has the ability to create animations\footnote{There are other formats such as \code{APNG} (Animated Portable Network Graphics), \code{MNG} (Multiple-image Network Graphics) and \code{SVG} (Scalable Vector Graphics), etc, but less popular (SVG might be promising in animations).}, nevertheless ultimately I didn't adopt this format as the primary means for creating animations because of several reasons\footnote{You may read this page: \url{http://r.yihui.name/misc/gif_pdf_grDev.htm}}. Anyway, it will be briefly introduced in section \ref{tool:other}. 
...
For detailed instructions and explanations in \R{} graphics system, you may refer to the book ``R graphics'' (\cite{BK:RG}) by Paul Murrell, or read R-help on graphics functions (e.g. packages \code{graphics}, \code{grDevices}, etc) carefully. 
...
As mentioned above, we can take \code{GIF} images as a medium for the expression of animations. However, it is likely that we need a certain special software to help with the generation of \code{GIF} images. The function \fun{saveMovie()} in the package ``\code{animation}'' has provided a wrapper to create \code{GIF} animations with the help of ``\href{http://www.imagemagick.org}{\code{ImageMagick}}'':
...
There are some common arguments in each animation function controlling the way to make animations, for example, whether to save \code{PNG} files during the animation demonstration (\para{saveANI}), the time interval \para{interval} between each frame of a whole animation, and the \para{height} and \para{width} of the animation frames if they are to be saved. These arguments are controlled by a special function \fun{ani.control()}: 
...
Beside the approach of \code{HTML} animation pages, the function \fun{saveMovie()} also provides another way for animations as mentioned before. Actually we may generate movies of any formats as long as they are supported by ``\code{ImageMagick}'', such as \code{GIF} (*.gif) or \code{MPEG} (*.mpg). Here are two examples:
...
Probability theory is a subject relevant to randomness. As mentioned in section \ref{random}, animation can be closely related to this subject.
...
\begin{figure}
  ...
  \end{center}
\end{figure}
In the first class of learning probability we usually begin with the probability in flipping coins or tossing dice, and the function \fun{flip.coin()} gives a simple simulation. Here the concept of a ``coin'' is actually abstract: it can be anything, and you just have to specify the true probabilities (or take the default \para{NULL}) for each ``face''. 
...
\begin{figure}[t]
  ...
  \caption[A demonstration of Law of Large Numbers.]{A demonstration of Law of Large Numbers. For the sample size $i$ changing from 1 to $n=300$: compute the sample mean for $i$ random numbers from the Bernoulli($p$) distribution with $p=0.2$, repeat this step for 10 times to get 10 means and plot them. Apparently, as the sample size $n$ grows, the sample mean will approach to the true mean $p$.}\label{lln}
  \end{center}
\end{figure}
...
The function \fun{lln.ani()} can illustrate this phenomenon in animations: for a series of sample sizes (denoted by $i$) from 1 to $n$, check the behaviour of the sample mean corresponding to this sample size $i$ by sampling \para{np} times from a certain distribution (given by the argument \para{FUN}). According to LLN, the sample mean will finally approach to the population mean (specified by the argument \para{mu}). Figure \ref{lln} has revealed this fact. 
...
Note that the arugment \para{FUN} is a function with two arguments \para{n} and \para{mean}, i.e. in the form of \code{FUN(n, mean)}. We may extend the distribution in this function by defining custom functions like:
...
The function \fun{clt.ani()} provides a demonstration for CLT in the following schema: first of all, \para{n} sample points are generated from a certain distribution, and the ecdf (empirical cumulative distribution function) as well as the pdf (estimated from \fun{density()}) is plotted. Then the sample mean is computed, and at last the density of these sample means is plotted as the replication of the generation of random numbers from that distribution. The whole plot area is divided into three parts to show the ecdf and pdf of the sample points from the corresponding replication and the pdf of the sample means estimated from all the replications. 
...
Figure \ref{CLT} shows a simulation for samples from the Uniform distribution. Other distributions such as Cauchy distribution (which has no finite variance\footnote{This is an interesting example to show a non-Gaussian distribution when the conditions are not satisfied.}) or Log Normal distribution may also be tried. 
...
\begin{figure}[t]
  ...
  \caption[The interpretation of confidence intervals.]{The interpretation of confidence intervals. Take samples from the Normal distribution $N(\mu, \sigma)$ (with $\sigma$ known) for example: the confidence interval for the population mean $\mu$ is $[\bar{x}-z_{\alpha/2}\sigma/n,\bar{x}+z_{\alpha/2}\sigma/n]$, so every time we get a sample we can compute such an interval. If we keep on sampling from $N(\mu, \sigma)$, we may obtain a series of intervals because $\bar{x}$ will change according to different samples. Some intervals may cover the true mean, while other ones may not. However, in the end, we can see that this coverage rate will be close to $1-\alpha$. In this example, the coverage rate is $47/50=94\%$ and the true $1-\alpha=95\%$.}\label{confint}
  \end{center}
\end{figure}

The concept of confidence intervals is quite important in mathematical statistics, but it is also confusing at the same time. A typical wrong interpretation for a confidence interval is that $P\{L(\mathbf{x})<\theta<U(\mathbf{x})\}=1-\alpha$ where $\mathbf{x}$ denotes the observations. For the \emph{observed} values of a sample, this interval is \emph{non-random}, so there is no meaning speaking of the probability of coverage, as the probability for a fixed interval to cover the true parameter is either 1 or 0. The fact is that the intervals are random quantities, i.e. functions of the random $\mathbf{X}$ rather than the observed $\mathbf{x}$. Based on this point, the function \fun{conf.int()} gives a illustration for the interval estimation of the parameter $\mu$ from $N(\mu, \sigma)$. For further knowledge of confidence intervals, please refer to textbooks such as \cite{BK:SI}. 
...
Figure \ref{confint} is the result of drawing samples from $N(0, 1)$ for 50 times and plotting the corresponding intervals with segments. Obviously some segments cover the true parameter 0 and some do not. Theoretically there should be $50*0.95=47.5$ intervals convering the true parameter. 
...
One of the characteristics of computational statistics is that it often involves with iterations and random numbers, which have been mentioned in section \ref{iteration} and \ref{random} respectively. A large proportion of demonstrations in this section have shown these two features (especially iterations). Usually the frames of animations are in accordance with iterations in the algorithms. 

\subsection{Bisection Method}
\begin{figure}
  \begin{center}
    \includegraphics[width=\textwidth]{bisection}
  \caption[Bisection method for root-finding on an interval]{Bisection method for root-finding on an interval. The equation to solve is $x^3 - 7x - 10 = 0$, and the initial interval is $[-3, 5]$. }\label{bisection}
  \end{center}
\end{figure}
...
Figure \ref{bisection} is an example for finding the root of the equation $x^3 - 7x - 10 = 0$. 
...
\begin{figure}[t]
  ...
  \caption[Gradient descent algorithm for a 2-D case]{Gradient descent algorithm for a 2-D case: $z(x, y) = x^2 + 2y^2$. By default, the iteration starts at the point $(-3, 3)$. The arrows are indicating the detailed process of minimization. Theoretically the point $(0, 0)$ is the best solution, and we can see from the figure that the solution in the end is quite close to this point.}\label{graddesc}
  \end{center}
\end{figure}
...
for $\gamma > 0$ a small enough number, then $F(\mathbf{a})\geq F(\mathbf{b})$. With this observation in mind, one starts with a guess $\mathbf{x}_0$ for a local minimum of $F$, and considers the sequence $\mathbf{x}_0, \mathbf{x}_1, \mathbf{x}_2, \ldots$ such that
...
The function \fun{grad.desc()} has provided a visual illustration for this process of minimization:
...
Note that the iteration will stop if the change in the \para{FUN} values is smaller than the threshold \para{tol}. Figure \ref{graddesc} shows a illustration of minimizing the function $z(x, y) = x^2 + 2y^2$. Actually we may try even more complex function, for example:
...
\begin{figure}
  ...
  \caption[Newton-Raphson method to solve an equation]{Newton-Raphson method to solve an equation $e^{-x} x = 0$ from a starting value $x_0 = 2$.}\label{newton}
  \end{center}
\end{figure}
...
The function \fun{newton.method()} is a very simple implementation of Newton's method. The iteration in the code follows this formula: 

\begin{equation}
    x_{k + 1} = x_{k} - \frac{f(x_{k})}{f'(x_{k})}
\end{equation}

From the starting value $x_{0}$, blue vertical lines and red points are plotted to show the location of the sequence of iteration values $x_1, x_2, \ldots$; red tangent lines are drawn to illustrate the relationship between successive iterations; the iteration values are in the right margin of the plot.
...
Figure \ref{newton} shows the process of finding the root of $e^{-x} x = 0$ by Newton's method from a starting value $x_0 = 2$. 
...
The blue points denote the original dataset, while the red points with (possible) leaves denote sample points being resampled; the number of leaves in the sunflower scatter plot just means how many times these points are resampled, as bootstrap samples \emph{with} replacement. 
...
Figure \ref{crossvalidation} shows a possible partition of the whole data set into a training set and test set (10-fold cross-validation). The test set can move from the first part to the last part, and this is the base for animations. 
...
I'm grateful to \href{http://www.stat.auckland.ac.nz/~paul/}{Dr Paul Murrell} for his instructions and suggestions on the initial idea of my \code{animation} package. I'd like to thank \href{http://www.bfro.uni-lj.si/MR/ggorjan/}{Gregor Gorjanc} for reminding me of creating an RSS feed for my web site \url{http://R.yihui.name}, and \href{http://wwwmaths.anu.edu.au/~johnm/}{John Maindonald} for valuable comments as well as a large number of ideas for statistical animations (I'm still working on them). 
