...
plot(x <- c(1:4, 4:1), y <- rep(2:1, each = 4), ann = F, 
    type = "n", axes = F, xlim = c(0.55, 4.45), ylim = c(0.55, 
        2.45), xaxs = "i", yaxs = "i") 
rect(x - 0.45, y - 0.45, x + 0.45, y + 0.45, border = "gray") 
...
\begin{itemize}
  ...
  \item Generally speaking, no additional programs are needed in order to display the animations, as long as the web browser supports JavaScript -- surely most browsers can meet such a simple requirement.
\end{itemize}
